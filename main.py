...
class App(ctk.CTk):
	...
	def manipulate_image(self, *args):
		...
		if effect == 'Emboss': self.image = self.image.filter(ImageFilter.EMBOSS)
		elif effect == 'Find edges': self.image = self.image.filter(ImageFilter.FIND_EDGES)
		elif effect == 'Contour': self.image = self.image.filter(ImageFilter.CONTOUR)
		elif effect == 'Edge enhance': self.image = self.image.filter(ImageFilter.EDGE_ENHANCE_MORE)
